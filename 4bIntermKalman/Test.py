...
log_s_Q_s = Slider(ax_Q_s, 'log(Q_s)', -3, 6, valinit=np.log10(Q_s), color = filter_color)  # Axes for slider, label, min, max, initial value
log_s_Q_v = Slider(ax_Q_v, 'log(Q_v)', -3, 6, valinit=np.log10(Q_v), color = filter_color)
...
def update(val):
    Q_s = 10**log_s_Q_s.val
    Q_v = 10**log_s_Q_v.val
    R = 10**log_s_R.val
    P_0_s = s_P0_s.val
    P_0_v = s_P0_v.val
    std = s_std.val
...
log_s_Q_s.on_changed(update)
log_s_Q_v.on_changed(update)
